...
@if($input == 'file')
    ...
    <script type="text/jsx">
        React.render(
            <FilesSlots files="{{$crud->form->row ? $crud->form->row->attachmentsByColumn($column->name)->toJson() : '[]'}}" empty_slots="{{$column->file_empty_slots}}" name="{{$column->name}}" target="/files/upload?_token={{csrf_token()}}"/>,
