...
@if($input == 'chosen')
    <select class="chosen" id="{{$column->name}}" name="{{$column->name}}" {!!$column->inputAttr($crud->form->getValue($column))!!}>
        <option value="">{{$column->input_label}}</option>
        @foreach($crud->form->value($column->options) as $k => $v)
            <option value="{{$k}}" {{$crud->form->getValue($column) == $k ? 'selected' : ''}}>{{$v}}</option>
        @endforeach
...
@if($input == 'multiselect')
    <select data-placeholder="{{$column->input_label}}" class="multiselect" id="{{$column->name}}" name="{{$column->name}}[]" {!!$column->inputAttr($crud->form->getValue($column))!!} multiple>
        @foreach($crud->form->value($column->options) as $k => $v)
...
@if($input == 'text')
    <div class="ui fluid {{$column->input_action ? ' action' : ''}}{{$column->input_icon ? ' left icon' : ''}}{{$column->labeled ? ' labeled' : ''}}{{$column->labeled_right ? ' labeled' : ''}} input">
        ...
        @endif
        <input {!!$column->inputAttr($crud->form->getValue($column))!!} type="text" id="{{$column->name}}" name="{{$column->name}}" {{$column->required ? 'required' : ''}}"/>
